...
function dereceHesapla2(results,hareket){
    ...
    if(results.poseLandmarks[27]){    
        if(hareket==1){
            ...
            var a =  (results.za[16].x - results.za[12].x);
            var b =  (results.za[16].y - results.za[12].y);
            ...
            canvasLine(results.za[16],results.za[12]);
        ...
        //matris
            ...
            var sanalnokta={
                x:results.za[23].x,
                y:-(results.za[23].y*1.25),
            };
                var angle=  Math.abs(90-dereceHesapla(sanalnokta,results.za[23],results.za[11]));
                var angle2=  Math.abs(90-dereceHesapla(sanalnokta,results.za[23],results.za[15]));
                canvasText(angle,"blue",canvasElement.width*0.39,canvasElement.height*0.70);//derece ekranda gösterme
...
function onResults(results) {
    ...
    canvasCtx.restore();
    if (results.za) {
        grid.updateLandmarks(results.za, POSE_CONNECTIONS, [
...
const pose  = new mpHolistic.Holistic(config);
